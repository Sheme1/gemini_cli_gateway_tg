...
async def transcribe_voice(audio_bytes: bytes, api_key: str, model: str = "gemini-2.5-flash") -> str:
...
async def voice_handler(
    ...
    if not config.gemini_api_key:
        await message.reply("❌ Для голосовых сообщений требуется GEMINI_API_KEY в .env")
        return
