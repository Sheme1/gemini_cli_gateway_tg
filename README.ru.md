...
- Модельные пресеты на каждого пользователя: `cheap`, `fast`, `balanced`, `quality`
...
| `STREAM_UPDATE_INTERVAL` | Нет | Минимальная пауза между обычными edit-сообщениями после первого чанка ответа. |
...
| `/sessions` | Показывает сохранённые сессии Gemini: открыть, удалить или экспортировать TXT |
...
`STREAM_MIN_UPDATE_CHARS`, чтобы не упираться в Telegram flood-limit.

Выбор модели хранится отдельно для каждого Telegram-пользователя в
`GATEWAY_STATE_DIR`; модель из `.env` остаётся fallback. Счётчики usage пишутся в
