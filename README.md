...
to Gemini CLI through `--include-directories` and appends native `@{full_path}`
file references to the user's prompt. The gateway does not change the selected
model and attachment prompts continue the same saved chat session as text
prompts. PDFs, images, audio, and video are handled by Gemini CLI's native file
reading. DOCX and text-like files also get a `.txt` sidecar extracted by the
gateway and referenced as another `@{full_path}` line. Other binary files are
accepted up to the Telegram Bot API download limit, but understanding depends on
